...
class restapi(Resource):
    ...
    def get(self):
        ...
        results_by_title = Book.query.filter(Book.title.ilike(f"%{phrase}%")).all()
        results_by_authors = Book.query.filter(Book.authors.ilike(f"%{phrase}%")).all()
        results_by_language = Book.query.filter(
            Book.language.ilike(f"%{phrase}%")
        ).all()

        results = list(set(results_by_title + results_by_authors + results_by_language))
